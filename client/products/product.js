  ...
  if (Array.isArray(arrOrden)) {
    ...
    const ordenCard = `
      <div class="carro" id=${orden.id}>
        <div class="carro-img">
        <button class="delete-mesa"><i class="fa fa-trash"></i></button>
        ...
        <button class="cobrar-mesa">Cobrar Mesa</button>
      </div>
    `;
    ...
    if (mesa) {
      deleteProductosDeLaOrden(mesa.id)
        .then(()=> window.location.reload())
        .catch((error) => console.error(error)) // Llama a la función para eliminar la mesa
    }
  });
...
const botonesCobrarMesa = document.querySelectorAll('.cobrar-mesa');
botonesCobrarMesa.forEach((boton) => {
    console.log(productos);
    const arrOrden = productos
    boton.addEventListener('click', (e) => {
      const mesa = boton.parentElement.querySelector('.carro-img p').textContent;
      
      if (e.target.closest('.carro')){
        const idOrden = e.target.closest('.carro').id;
      const totalVentas = e.target.closest('.carro').children[2].children[0].innerText.split(' ')[1];
      // const deletedMesa = e.target.closest('.carro').children[0].children[0]
      ...
      enviarVentasABaseDeDatos(datos)
      .then(() => {
        console.log(`Venta agregada a la base de datos`);
      })
        .then(() => {
          deleteProductosDeLaOrden(idOrden)
          .then(() => {
            // quitar refrescar
              window.location.reload()
            })
          })
          .catch((error) => console.error('[enviarVentasABaseDeDatos]: Error al crear orden', error));
        } else {
...
// obtener Ventas
const botonVentas = document.getElementById('ventasboton');

botonVentas.addEventListener('click', () =>{

  if(!cuerpoTablaVentas.classList.contains('abrir-ventas')){
    cuerpoTablaVentas.classList.add('abrir-ventas');

    obtenerVentas()
    .then((ventasObtenidasDesdeLaBaseDeDatos) => {
      const ventasParaLaTabla = ventasObtenidasDesdeLaBaseDeDatos.message.map((ventas) => {
        console.log(ventas.ventasEnDB)
        const datosVentas = `
        <tr>
          <td class="tableBody">${ventas.ventasEnDB.mesa}</td>
          <td class="tableBody">${ventas.ventasEnDB.total}</td>
        </tr>
        `;
        return datosVentas;
      })
      ...
      const tablaVentas = `
      <table class="ventas" id="ventas">
        <thead>
          <th>Mesa</th>
          <th>Total</th>
        </thead>
        <tbody id="tablaVentas">
          ${ventasParaLaTabla.join('')}
        </tbody>
      </table>
      `;
  
      cuerpoTablaVentas.innerHTML = tablaVentas;
    })
    .catch((error) => console.log(error))
  
      .catch((error) => console.log(error))
  } else {
    cuerpoTablaVentas.classList.remove('abrir-ventas')
